...
// Sprint 66 — exchange a Firebase ID token for a Ziza JWT (role: admin).
// admin_code is required by the backend only when creating a NEW admin.
export async function exchangeFirebaseToken(idToken, { adminCode, name } = {}) {
...
export async function signup(email, password, name, adminCode) {
  const res = await fetch(`${API_BASE}/v1/auth/signup`, {
    method: "POST",
    headers: { "Content-Type": "application/json" },
    body: JSON.stringify({ email, password, name: name || null, role: "admin", admin_code: adminCode }),
  });
  return _json(res);
}
