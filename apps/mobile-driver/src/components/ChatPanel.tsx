 ...
 * Driver ↔ customer, scoped to a trip. NOT shared across apps (isolation rule).
 */
import React, { useEffect, useRef, useState } from "react";
import { View, Text, TextInput, TouchableOpacity, ScrollView, StyleSheet } from "react-native";
import { listTripMessages, sendTripMessage, ChatMessage } from "../api";
...
export default function ChatPanel({ token, tripId, accent = "#1D4ED8" }: Props): React.ReactElement {
  ...
  useEffect(() => {
    let active = true;
    const load = () =>
      listTripMessages(token, tripId).then((m) => { if (active) setMessages(m); }).catch(() => {});
    load();
