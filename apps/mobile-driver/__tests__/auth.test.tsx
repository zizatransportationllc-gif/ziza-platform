...
test("login() stores access token in AsyncStorage after successful response", async () => {
  mockFetch.mockResolvedValueOnce({
    ok: true,
    json: async () => ({
      access_token: "drv_tok_abc123",
      refresh_token: "drv_ref_abc123",
      token_type: "bearer",
      expires_in: 3600,
    }),
  });
...
test("clearToken() removes the stored driver token from AsyncStorage on logout", async () => {
  await storeToken("drv_tok_xyz789");
  await clearToken();
