...
function LoginForm({ onSuccess, error, loading }) {
  ...
  return (
    <div className="app">
      ...
      <form className="login-form" onSubmit={(e) => { e.preventDefault(); onSuccess(email, password); }}>
        <input type="email" value={email} onChange={(e) => setEmail(e.target.value)} placeholder="Email" required />
        <input type="password" value={password} onChange={(e) => setPassword(e.target.value)} placeholder="Password" required />
        <button type="submit" disabled={loading}>{loading ? "Connexion…" : "Se connecter"}</button>
        {error && <p className="form-error">{error}</p>}
      </form>
      <p className="hint">Dev users: driver@ziza.dev · Password: ziza2024</p>
    </div>
  );
}
...
function Dashboard({ user, demo, onLogout }) {
  return (
    <div className="app">
      ...
      <p className="footer">App: <strong>web-driver</strong> · Sprint 2</p>
    </div>
  );
}
