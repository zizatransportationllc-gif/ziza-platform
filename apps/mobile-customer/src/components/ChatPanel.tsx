 ...
 * scope="trip" → customer ↔ driver; scope="request" → customer ↔ professional.
 * NOT shared across apps (isolation rule).
 */
...
export default function ChatPanel({ token, scope, id, accent = "#1D4ED8" }: Props): React.ReactElement {
  ...
  const listFn = scope === "trip" ? listTripMessages : listRequestMessages;
  const sendFn = scope === "trip" ? sendTripMessage : sendRequestMessage;

  useEffect(() => {
    let active = true;
    const load = () => listFn(token, id).then((m) => { if (active) setMessages(m); }).catch(() => {});
    load();
  ...
  const handleSend = async () => {
    ...
    setSending(true);
    try {
      const msg = await sendFn(token, id, body);
