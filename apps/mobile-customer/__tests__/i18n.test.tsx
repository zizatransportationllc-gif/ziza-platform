...
test.each(["login.", "assistance."])("every en %s* key has a matching es key (and vice versa)", (prefix) => {
...
test("login.verifyNotice interpolates the email placeholder in both languages", () => {
  expect(interpolate(translations.en["login.verifyNotice"], { email: "a@b.com" })).toContain("a@b.com");
  expect(interpolate(translations.es["login.verifyNotice"], { email: "a@b.com" })).toContain("a@b.com");
});
...
test("a missing key falls back to the key itself (regression guard on existing t() behavior)", () => {
  ...
  expect(lookup("login.doesNotExist")).toBe("login.doesNotExist");
});
